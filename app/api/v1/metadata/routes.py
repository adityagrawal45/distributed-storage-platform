...
Phase 13: every route depends on `CurrentOrganization` — see
`app/api/v1/folders/routes.py`'s module docstring for the full rationale,
identical here.
"""
...
async def create_metadata(
    payload: FileMetadataCreate, current_user: CurrentUser, org: CurrentOrganization, metadata_service: MetadataServiceDep
) -> APIResponse[FileMetadataRead]:
    file = await metadata_service.create_metadata(current_user.id, org.organization_id, payload)
...
async def search_files(
    ...
    page = await search_service.search_files_page(
        current_user.id, org.organization_id, search_params, pagination.page, pagination.page_size
    )
...
async def list_file_trash(
    current_user: CurrentUser, org: CurrentOrganization, metadata_service: MetadataServiceDep
) -> APIResponse[list[FileMetadataRead]]:
    files = await metadata_service.list_trash(current_user.id, org.organization_id)
    return APIResponse(
...
async def get_metadata(
    file_id: uuid.UUID, current_user: CurrentUser, org: CurrentOrganization, metadata_service: MetadataServiceDep
) -> APIResponse[FileMetadataRead]:
    file = await metadata_service.get_metadata_cached(file_id, current_user.id, org.organization_id)
...
async def update_metadata(
    ...
    file = await metadata_service.update_metadata(file_id, current_user.id, org.organization_id, payload, current_user.id)
...
async def rename_file(
    ...
    file = await metadata_service.rename_file(file_id, current_user.id, org.organization_id, payload.name, current_user.id)
...
async def move_file(
    ...
    file = await metadata_service.move_file(
        file_id, current_user.id, org.organization_id, payload.new_folder_id, current_user.id
    )
...
async def delete_file(
    ...
    await metadata_service.delete_file(file_id, current_user.id, org.organization_id, current_user.id)
...
async def restore_file(
    ...
    file = await metadata_service.restore_file(file_id, current_user.id, org.organization_id, current_user.id)
...
async def permanent_delete_file(
    ...
    await metadata_service.permanent_delete_file(file_id, current_user.id, org.organization_id)
...
async def get_file_versions(
    file_id: uuid.UUID, current_user: CurrentUser, org: CurrentOrganization, version_service: VersionServiceDep
) -> APIResponse[list[FileVersionRead]]:
    versions = await version_service.list_versions_cached(file_id, current_user.id, org.organization_id)

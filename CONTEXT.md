...
Purpose of this file: give a fresh AI session (or human) full context on this project in one read, without needing to re-explore the codebase from scratch. Written 2026-08-04, updated same day after completing Phase 3.

## Current Status: Phases 1–3 complete, repo healthy
...
- Full test suite: **76/76 passing** (57 Phase 1/2 + 19 Phase 3), against in-memory SQLite (`aiosqlite`) — no external services needed to run `pytest`.
...
**Not yet built** (future phases, per README §19): chunked/resumable uploads, sharing/permissions between users, virus scanning (placeholder only), thumbnails, full-text content search, Pub/Sub background workers, rate limiting, Redis caching, CI/CD, Terraform/GKE deployment, observability/metrics.
...
```
app/
  main.py                    create_application() factory, lifespan, middleware, exception handlers, mounts api_router
  api/v1/
    ...
    health/routes.py           /health endpoint
  core/
    config/settings.py         Settings, get_settings() — includes GCS_*, MAX_UPLOAD_SIZE_MB, ALLOWED_MIME_TYPES, BLOCKED_EXTENSIONS
  ...
  database/
    session.py                  async engine/session, declarative Base
    redis.py                    redis pool + check_redis_connection()
    gcs.py                      GCS client factory (ADC in prod, key file path in dev) — Phase 3
  dependencies/
    auth.py                     get_current_user, CurrentUser, require_role()
    providers.py                DI wiring for repositories/services, incl. StorageServiceDep/FileUploadServiceDep/GCSClientDep
  ...
  schemas/                      auth.py, user.py, folder.py, file_metadata.py (+FileUploadResponse/SignedUrlResponse), health.py,
                                 pagination.py, response.py (APIResponse[T] envelope), search.py, sorting.py
  exceptions/                   custom_exceptions.py (+Storage* exceptions), handlers.py (+Storage*/FileTooLarge/UnsupportedFileType handlers)
  logging/logger.py             structlog config, get_logger()
  middleware/                   request_context.py, security_headers.py
...
tests/
  conftest.py                  client/db fixtures + fake_gcs_client fixture (overrides get_gcs_client for every test)
...
All Phase 1/2 vars (see README §12) plus Phase 3: `GCS_PROJECT_ID`, `GCS_BUCKET_NAME`, `GCS_CREDENTIALS_PATH` (leave unset outside local dev — ADC/Workload Identity is used instead), `SIGNED_URL_EXPIRATION_MINUTES`, `MAX_UPLOAD_SIZE_MB`, `ALLOWED_MIME_TYPES`, `BLOCKED_EXTENSIONS`.

## Tests

76/76 passing. Run with `pytest -v`. Phase 3 tests never touch real GCS — `tests/fakes/fake_gcs.py::FakeGCSClient` is wired in via `app.dependency_overrides[get_gcs_client]` in `conftest.py`'s `client` fixture, so every test (not just storage ones) gets the fake transparently.
...
Resume roadmap work at **Phase 4** per README §19 — the user plans to provide the Phase 4 prompt in a future session. Do not regenerate Phases 1–3; extend the existing codebase only.
